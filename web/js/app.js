...
// Motor compartido para análisis (se crea una vez).
let analysisEngine = null;
function getAnalysisEngine() {
  if (!analysisEngine) analysisEngine = new Engine("js/engine/stockfish.js");
...
async function analyze() {
  ...
  try {
    ...
    const result = await analyzeGames(pgn, depth, engine, (done, total) => {
      setProgress(done, total, `Analizando jugada ${done}/${total}`);
    });
    onResult(result);
  } catch (err) {
